...
namespace ReconKitApp
{
    ...
    public partial class AdminsForm : Form
    {
        ...
        {
            if (currentLevel==3) ShowEditor();
            else MessageBox.Show("No autorizado: nivel 3 requerido");
        }

        private void btnEdit_Click(object sender, EventArgs e)
        {
            if (currentLevel>=2) EditSelected();
            else MessageBox.Show("No autorizado: nivel 2 requerido");
        }
        ...
        private void RefreshGrid()
        {
            ...
            {
                c.Open();
                using (var da = new SQLiteDataAdapter("SELECT id, username, level FROM admins", c))
                {
        ...
        {
            var dlg = new AdminEditorForm(id);
        ...
        private void DeleteSelected()
        {
            if (dgv.SelectedRows.Count==0) { MessageBox.Show("Seleccione una fila"); return; }
            var id = Convert.ToInt64(dgv.SelectedRows[0].Cells["id"].Value);
            if (MessageBox.Show("Eliminar admin ID "+id+"?", "Confirmar", MessageBoxButtons.YesNo)==DialogResult.Yes)
            {
                using (var c = new SQLiteConnection(DataAccess.ConnectionString))
                {
                    c.Open();
